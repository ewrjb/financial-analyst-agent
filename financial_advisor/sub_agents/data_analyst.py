...
def get_company_info(ticker: str) -> str:
    ...
    info = stock.info
    # 일반적인 string 으로 return해도 되지만 Google ADK에서는 구조화된 형식으로 return하는 것을 권장하고 있음.
    # 에러가 난 경우에도 success : False, reason : ...과 같이 구체적인 정보를 제공하는 것이 최종 성능에 긍정적인 영향을 미침.
    return {
        "ticker": ticker,
        "success": True,
...
data_analyst = LlmAgent(
    name="DataAnalyst",
    model=MODEL,
    # description은 root agent가 sub agent를 선택할 때 참고하는 용도.
